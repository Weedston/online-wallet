    ...
    for path in paths:
        for root, dirs, filenames in os.walk(path):
            for dir_name in dirs:
                directories.append(os.path.relpath(os.path.join(root, dir_name), start="."))
            for file_name in filenames:
                files.append(os.path.relpath(os.path.join(root, file_name), start="."))
...
def generate_file_structure(paths, indent=0):
    ...
    for directory in sorted(directories):
        structure += " " * indent + f"├── {directory}\n"
    
    for file in sorted(files):
        structure += " " * indent + f"├── {file}\n"
...
if __name__ == "__main__":
    ...
    folders_to_scan = ["pages/p2p", "config", "css"]
